...
1. You can use RStudio IDE's Visual Markdown editor and in the project/global options, choose "sentence" as value for "Automatic text wrapping (line breaks)" under "R Markdown".

## How to fix a file that has no linebreaks at the end of sentences?

1. Open it with RStudio IDE's Visual Markdown editor with the correct R Markdown options, then save.
1. Use aeolus.
...
aeolus uses the tinkr package under the hood, which means it suffers from the same limitations: for instance, files after editing only use "-", not "*", for lists.
